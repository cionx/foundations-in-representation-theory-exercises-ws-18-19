...
Let~$g \colon \Dcc \to \Ccc$ be a morphism of~\dash{$\Acat$}{valued} chain complexes and let~$h \colon X \to Y$ be a morphism in~$\Acat$.
Then the diagram
\begin{equation}
...
\[
  ...
  \end{tikzcd}
\]
The four trapezoids commute by the construction of~$L$ and~$\induced{(-)}$.
